...
### 🚀 LAB-6: Bütünleşik Uygulama & React Temelleri (Güncel)
Projenin en üst katmanı olan React component'leri (`Hero`, `ContactForm`, `ProjectList` vb.) kuruldu.
- **Performans:** `useMemo` kullanılarak gereksiz filtreleme işlemlerinin önüne geçildi.
- **Controlled Forms:** İletişim formu anlık state takibi ile (Controlled Component) yazıldı ve doğrulama eklendi.
- **İş Akışı:** TypeScript `verbatimModuleSyntax` kurallarına uyan strict mode build'leri geçildi.

### 📌 LAB-5: TypeScript Temelleri ve Gelişmiş State Yönetimi
...
### 🚀 LAB-6: Checkpoint #1 Uygulama Demosu
Uygulamanın hero alanı, dinamik projeler filtrelemesi ve state kontrollü iletişim formunun canlı etkileşimi.

![Checkpoint 1 Demo](./screenshots/lab6_demo.webp)
